...
namespace KaraokeNeeNee
{
    public partial class CapNhatTB : Form
    {
        ...
        {
            con = new SqlConnection(@"Data Source=CHINHCHINT\MSSQLSERVER01;Initial Catalog=KARAOKE_NEENEE;Integrated Security=True");
            if (con.State == ConnectionState.Closed)
                con.Open();
        }
        public void ShowTBInfo(string maTB)
        {
            using (SqlConnection connection = new SqlConnection(@"Data Source=CHINHCHINT\MSSQLSERVER01;Initial Catalog=KARAOKE_NEENEE;Integrated Security=True")) 
            {
                // Mở kết nối
                connection.Open();
